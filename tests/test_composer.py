...
def test_find_lua_files_basic(tmp_path):
    ...
    expected_files = sorted([
        project_dir / "file1.lua",
        project_dir / "sub" / "file3.lua"
    ])
...
''')
    assert composer.parse_dependencies(file_path, src_dir) == {"module.one", "module.two"}
...
def test_sanitize_goto_failure(tmp_path):
    ...
    content = "local x = 1\ngoto mylabel\n::mylabel::"
    with pytest.raises(Exception, match=r"Disallowed 'goto' statement found in .*?test_goto.lua on line 2: goto mylabel"):
...
def test_sanitize_loadlib_removed_with_warning(tmp_path, capsys):
    ...
    sanitized = composer.sanitize_content(content, file_path, is_lua_module=True, dcs_strict_sanitize=True) # strict flag doesn't affect loadlib warning/removal
    assert sanitized.strip() == expected_content.strip() # Use strip to handle potential subtle newline differences
    
    captured = capsys.readouterr()
    assert f"WARNING: [{file_path}] Disallowed 'loadlib' call found and was removed: loadlib('some.path.dll', 'luaopen_somelib')" in captured.out
...
def test_topological_sort_simple_chain(tmp_path):
    ...
    for p in module_to_path_map.values():
        p.touch()
    assert composer.topological_sort(graph_complete, modules, module_to_path_map) == ["A", "B", "C"]
...
def test_build_project_basic_composition(tmp_path, sample_project_structure_basic, mocker):
    ...
    assert re.search(r"ProjectNS\.app_start\s*=\s*function\(\s*\)\s*env\.info\(\"App Start\"\)\s*end", content), "Transformed app_start function not found or incorrect"
...
def test_build_project_complex_functional(tmp_path, mocker):
    ...
    mocked_now = datetime.datetime(2023, 10, 27, 11, 0, 0, tzinfo=datetime.timezone.utc) # Changed timestamp for new test
    mocker.patch('composer.datetime.datetime')
    composer.datetime.datetime.now.return_value = mocked_now
    ...
    assert re.search(r"if\s+MyProjectNS\.service_loaded\s+then.*?end", content, re.DOTALL), "Sanitized if block structure not found"
    ...
    for name in all_core_module_names:
        assert name in sorted_core_modules_str, f"{name} not found in Core Modules Order comment: {sorted_core_modules_str}"
...
# Add more tests for:
# - Circular dependencies in build_project context (using new complex structure)
# - Different sanitization cases within build_project
# - Optional header/footer not present / one present, one not
# - No core modules (only namespace and entrypoint)

# Add more tests for:
# - Circular dependencies in build_project context
# - Different sanitization cases within build_project
# - Optional header/footer not present
# - No core modules
# - Complex dependency chains

# OBSOLETE - DO NOT USE - delete this function
# def test_build_project_complex_functional(tmp_path, mocker):
#     # Path to the on-disk functional test project
#     base_test_dir = Path(__file__).resolve().parent
#     src_dir = base_test_dir / "functional_test_project" / "src"
    
#     output_file = tmp_path / "dist" / "complex_final_script.lua"

#     mocked_now = composer.datetime.datetime(2023, 10, 26, 10, 0, 0, tzinfo=composer.datetime.timezone.utc)
#     mocker.patch('composer.datetime.datetime')
#     composer.datetime.datetime.now.return_value = mocked_now
...
#         output_file=str(output_file),
#         header_file_rel="header.txt",
#         namespace_file_rel="my_namespace.lua",
#         entrypoint_file_rel="app_entry.lua",
#         footer_file_rel="footer.txt"
#     )
...
#     # 1. Check for header and footer (verbatim, print statements remain)
#     assert "-- FUNCTIONAL TEST HEADER --" in content
#     assert "print('Header print check')" in content # Verbatim
#     assert "-- FUNCTIONAL TEST FOOTER --" in content
#     assert "print('Footer print check')" in content # Verbatim

#     # 2. Check for build info
#     assert f"-- Combined and Sanitized Lua script generated on {mocked_now.isoformat()}" in content
#     assert "-- Header File: header.txt" in content
#     assert "-- Namespace File: my_namespace.lua" in content
#     assert "-- Entrypoint File: app_entry.lua" in content
#     assert "-- Footer File: footer.txt" in content

#     # 3. Check for namespace content (sanitized)
#     assert "MyGlobalNS = { project = \"ComplexTest\" }" in content
#     assert "print('Namespace print check')" not in content # Sanitized

#     # 4. Check for Core Modules (sanitized and in correct topological order)
#     # Expected content snippets from core modules (after sanitization)
#     common_utils_code = "MyGlobalNS.utils = { version = '1.0' }"
#     common_constants_code = "MyGlobalNS.constants = { PI = 3.14 }"
#     services_data_fetcher_code = "MyGlobalNS.data_fetcher = function()\n    return { data = \"sample\" }\nend"
#     services_main_logic_code = "MyGlobalNS.main_logic = function()\n    local data = MyGlobalNS.data_fetcher()\n    return data.data .. \" processed\"\nend"
    
#     features_helper_one_code = "MyGlobalNS.helper_one_version = \"v1.0.0\"\nMyGlobalNS.helper_one_func = function(input)\n    return \"HelperOne processed: \" .. tostring(input)\nend" # Sanitized print
    
#     features_mod_one_local_helper_code = "MyGlobalNS.mod_one_local_helper = function()\n    return MyGlobalNS.constants.PI * 2\nend"
#     features_mod_one_mod_one_code = "MyGlobalNS.mod_one_init = function()\n    local local_helper_value = MyGlobalNS.mod_one_local_helper()\n    local shared_helper_confirmation = MyGlobalNS.helper_one_func(\"ping from mod_one\")\n    return MyGlobalNS.main_logic() .. \" for module_one, helped by \" .. local_helper_value .. \" and shared: \" .. shared_helper_confirmation\nend"
    
#     features_mod_two_mod_two_code = "MyGlobalNS.mod_two_init = function()\n    local mod_one_msg = MyGlobalNS.mod_one_init()\n    local helper_msg = MyGlobalNS.helper_one_func(\"data for helper from mod2\")\n    return \"ModTwo got: [\" .. mod_one_msg .. \"] and [\" .. helper_msg .. \"]\"\nend"

#     core_module_contents = {
#         "common.utils": common_utils_code,
#         "common.constants": common_constants_code,
#         "services.data_fetcher": services_data_fetcher_code,
#         "services.main_logic": services_main_logic_code,
#         "features.helper_one": features_helper_one_code,
#         "features.module_one.mod_one_local_helper": features_mod_one_local_helper_code,
#         "features.module_one.mod_one": features_mod_one_mod_one_code,
#         "features.module_two.mod_two": features_mod_two_mod_two_code
#     }

#     # Verify all core module snippets are present and sanitized
#     for name, snippet in core_module_contents.items():
#         assert snippet in content, f"Snippet for {name} not found or incorrect (actual: {content[:500]}). Expected: {snippet[:100]}"
#         # Adjust print check based on actual module name part in print statements
#         if name == "features.helper_one":
#             assert "print('helper_one print')" not in content, f"Print from {name} not sanitized"
#         elif name == "features.module_two.mod_two":
#             assert "print('mod_two print')" not in content, f"Print from {name} not sanitized"
#         elif name not in ["common.utils", "common.constants", "services.data_fetcher", "services.main_logic", "features.module_one.mod_one_local_helper", "features.module_one.mod_one"]:
#              # Fallback for any new modules, assuming simple print naming
#              assert f"print('{name.split('.')[-1]} print')" not in content, f"Print from {name} not sanitized (fallback check)"
#         # Check original specific prints for modules whose names didn't change much
#         elif name == "common.utils": assert "print('utils print')" not in content, f"Print from {name} not sanitized"
#         elif name == "common.constants": assert "print('constants print')" not in content, f"Print from {name} not sanitized"
#         # For other modules, their specific print sanitization is implicitly checked by their full snippet matching if they had unique prints

#     # Check for correct ordering (more robustly)
#     idx_const = content.find(common_constants_code)
#     idx_utils = content.find(common_utils_code)
#     idx_data_fetcher = content.find(services_data_fetcher_code)
#     idx_main_logic = content.find(services_main_logic_code)
#     idx_helper_one = content.find(features_helper_one_code)
#     idx_mod_one_local_helper = content.find(features_mod_one_local_helper_code)
#     idx_mod_one = content.find(features_mod_one_mod_one_code)
#     idx_mod_two = content.find(features_mod_two_mod_two_code)

#     # Base dependencies
#     assert idx_const < idx_data_fetcher, "common.constants should be before services.data_fetcher"
#     assert idx_const < idx_mod_one_local_helper, "common.constants should be before features.module_one.mod_one_local_helper"
#     assert idx_utils < idx_main_logic, "common.utils should be before services.main_logic"
#     assert idx_utils < idx_mod_one, "common.utils should be before features.module_one.mod_one"

#     # Service layer dependencies
#     assert idx_data_fetcher < idx_main_logic, "services.data_fetcher should be before services.main_logic"
    
#     # Feature helper_one dependencies
#     assert idx_helper_one < idx_mod_one, "features.helper_one should be before features.module_one.mod_one"
#     assert idx_helper_one < idx_mod_two, "features.helper_one should be before features.module_two.mod_two"

#     # Feature module_one dependencies
#     assert idx_mod_one_local_helper < idx_mod_one, "features.module_one.mod_one_local_helper should be before features.module_one.mod_one"
#     assert idx_main_logic < idx_mod_one, "services.main_logic should be before features.module_one.mod_one"
    
#     # Feature module_two dependencies
#     assert idx_mod_one < idx_mod_two, "features.module_one.mod_one should be before features.module_two.mod_two"

#     # 5. Check for Entrypoint content (sanitized)
#     app_entry_code_sanitized = '''MyGlobalNS.start_app = function()
#     local mod_two_result = MyGlobalNS.mod_two_init()
#     return "App started with: " .. mod_two_result
# end'''
#     assert app_entry_code_sanitized in content
#     assert "print(\"App Entry now starting module two\")" not in content
#     assert "require \"features.module_two.mod_two\"" not in content
#     assert "local core_package_test = package.path" not in content 
#     assert "local core_package_test =" not in content 
...
#     expected_core_module_names = [
#         "common.constants", "common.utils", "services.data_fetcher", 
#         "features.helper_one", "features.module_one.mod_one_local_helper", 
#         "services.main_logic", "features.module_one.mod_one", "features.module_two.mod_two"
#     ]
#     for name in expected_core_module_names:
#         assert name in sorted_core_modules_str, f"{name} not found in Core Modules Order comment: {sorted_core_modules_str}"
#     assert len(sorted_core_modules_str.split(', ')) == len(expected_core_module_names), \
#         f"Incorrect number of modules in Core Modules Order. Expected {len(expected_core_module_names)}, Got {len(sorted_core_modules_str.split(', '))}. String: {sorted_core_modules_str}" 